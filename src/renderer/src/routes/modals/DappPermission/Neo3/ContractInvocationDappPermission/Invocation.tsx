...
export const Invocation = ({ invocation, session, blockchain }: TProps) => {
  ...
  return (
    <div className="bg-asphalt w-full rounded-sm px-4 pt-3 pb-5 text-sm text-gray-100">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <TbArrowsSort aria-hidden className="text-blue h-6 w-6 rotate-90" />
          <p className="text-white capitalize">{invocation.operation}</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <IconButton
            icon={<MdChevronRight aria-hidden className="text-gray-100" />}
            compacted
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex min-w-0 justify-between gap-3 rounded-sm bg-gray-700/60 py-2.5 pr-4 pl-5">
          ...
          {explorerUrl && (
            <IconButton icon={<MdLaunch aria-hidden className="text-neon" />} compacted onClick={handleHashClick} />
          )}
        </div>
      </div>
      {showAmount && (
        <div className="flex flex-col gap-1">
          <div className="mt-4 mb-4 h-px min-h-0.25 w-full bg-gray-300/30"></div>
